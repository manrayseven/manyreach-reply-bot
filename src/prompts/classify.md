...
0c. ⚠️ **REFUS EXPLICITE / ABSENCE DE BESOIN PRIME sur une mention incidente de fermeture ou d'indisponibilité**. Si le prospect dit clairement qu'il **n'est pas intéressé** OU qu'il **n'a aucun besoin** ("non je ne suis pas intéressé", "nous n'avons pas de besoin", "on ne prend de toute façon pas de réservations", "ça ne nous concerne pas", "pas pour nous"), c'est `not_interested_polite` — **même si la phrase mentionne aussi** une fermeture, une haute saison, un manque de disponibilité ou des circonstances ("nous sommes actuellement fermés", "c'est la haute saison", "pas disponibles en ce moment"). Ces mentions sont du **contexte**, pas une demande de recontact. → `objection_timing` est réservé aux cas où le prospect demande EXPLICITEMENT à être recontacté plus tard ("rappelez-moi en septembre", "recontactez-moi à la réouverture", "revenez vers moi dans 3 mois") SANS refus catégorique. En cas de doute entre les deux, et s'il y a un refus net + zéro demande de recontact → `not_interested_polite`.
   - Exemples `not_interested_polite` : "non je ne serais pas intéressé. Nous sommes actuellement fermés suite aux inondations" · "ne sommes pas disponibles en ce moment et nous ne prenons de toute façon pas de réservations".
