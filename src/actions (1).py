...
# Map intent -> (sendingStatus, sendingActive, [tags to add])
INTENT_PROSPECT_UPDATE: dict[str, tuple[str, bool, list[str]]] = {
    ...
    "bounce_or_auto":                  ("BounceHard",     False, ["bot:bounce-or-auto"]),
}

# Intents that are SAFE to auto-send (high reliability + low downside)
AUTOSEND_ELIGIBLE = frozenset({
    "interested_warm",        # only if confidence >= threshold AND slots present
    "not_interested_polite",  # short, generic acknowledgment
